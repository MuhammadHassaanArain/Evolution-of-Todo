...
def get_task(
    ...
    try:
        ...
        if not db_task:
            log_task_event("task_access_denied", user_id=current_user.id, task_id=task_id, success=False, details={"reason": "task_not_found"})
            raise HTTPException(
...
def delete_task(
    ...
    try:
        ...
        if not success:
            log_task_event("task_delete_failed", user_id=current_user.id, task_id=task_id, success=False, details={"reason": "task_not_found"})
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Task not found"
            )
