...
class SampleDataCreator:
    """
    Class to create sample data for testing and development.
    """
    ...
    def create_sample_users(self, count: int = 2) -> List[User]:
        ...
        with Session(self.engine) as session:
            existing_users = session.exec(select(User)).all()
            if existing_users:
                log_info(f"Found {len(existing_users)} existing users, skipping creation")
                self.created_users = existing_users
                return existing_users
        ...
        self.created_users = new_users
        log_info(f"Created {len(new_users)} sample users")
        return new_users

    def create_sample_tasks(self, count: int = 3) -> List[Task]:
        """
        Create sample tasks associated with users.
        """
        with Session(self.engine) as session:
            users = session.exec(select(User)).all()
            if not users:
            ...
            if existing_tasks:
                log_info(f"Found {len(existing_tasks)} existing tasks, skipping creation")
                self.created_tasks = existing_tasks
            ...
            for i in range(count):
                ...
                task = Task(
                    ...
                    is_completed=(i % 3 == 0),
                    user_id=user.id,  # Must match User.id
        ...
        log_info(f"Created {len(tasks)} sample tasks")
        return tasks
    ...
    def create_complete_sample_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log_info(f"Complete sample data created: {len(users)} users, {len(tasks)} tasks")
            return result
        except Exception as e:
            log_error(f"Error creating sample data: {str(e)}")
            raise
    ...
    def validate_sample_data(self) -> Dict[str, Any]:
        """
        Validate that all tasks have valid user references.
        """
        with Session(self.engine) as session:
            users = session.exec(select(User)).all()
            tasks = session.exec(select(Task)).all()
            valid_tasks = sum(1 for t in tasks if session.get(User, t.user_id) is not None)
            return {
                "users_count": len(users),
                "tasks_count": len(tasks),
                "valid_tasks": valid_tasks,
                "orphaned_tasks": len(tasks) - valid_tasks,
                "validation_passed": len(tasks) == valid_tasks
            }
    ...
    def clear_sample_data(self) -> Dict[str, Any]:
        ...
        log_info(f"Cleared sample data: {len(self.created_tasks)} tasks, {len(self.created_users)} users")
        return {
            "success": True,
            "users_deleted": len(self.created_users),
            "tasks_deleted": len(self.created_tasks)
        }
...
def create_sample_data() -> Dict[str, Any]:
    return sample_data_creator.create_complete_sample_data()

def validate_sample_data() -> Dict[str, Any]:
    return sample_data_creator.validate_sample_data()

def clear_sample_data() -> Dict[str, Any]:
    return sample_data_creator.clear_sample_data()
